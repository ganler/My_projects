...
if __name__ == '__main__':
    ...
    distributions = list_all()

    begin_time = time.time()  # 用于计算程序执行时间

    best_distribution = []
    most_pieces = 0
    if len(distributions)==256:
        for distribution in distributions:
            if distribution.tolist() == [1]*8 or distribution.tolist() == [2]*8:
                pass
            else:
                # 【参数列表】-- begin
                ...
                wash_time = 25
                err_rate = 0.0 # 故障率，0为无故障情况，1为有故障情况。

                # 【参数列表】-- end

                process2_completed = workpieces_in_time(8*60*60, distribution)
                if process2_completed > most_pieces:
                    most_pieces = process2_completed
                    best_distribution = distribution

    print('最佳分布', best_distribution)
    print('8小时内产生工件量', most_pieces)
